...
class WishlistController {
	...
	clearWishlist = asyncWrapper(async (req: AuthenticatedRequest, res) => {
		await wishlistService.clearWishlist(req.user);
	...
	deleteWishlist = asyncWrapper(async (req: AuthenticatedRequest, res) => {
		await wishlistService.deleteWishlist(req.user);
		res.json({ message: 'Wishlist deleted' });
	});

	addProductToWishlist = asyncWrapper(
		async (req: AuthenticatedRequest, res) => {
			await wishlistService.addProductToWishlist(req.user, req.params.id);
	...
	removeProductFromWishlist = asyncWrapper(
		async (req: AuthenticatedRequest, res) => {
			await wishlistService.removeProductFromWishlist(req.user, req.params.id);
